	...
	<view class="all">
		<view class="u-input">
			...
			<view class="username">
				...
				<button type="primary" class="update" size="default" @click="crewUpdate1" plain="true">修改</button>
				<button type="warn" size="default" class="delete"  @click="crewDelete1()" plain="true">删除</button>
		</view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		methods: {
			...
			async crewUpdate1(){
				const {code,data,message,success} = await crewUpdate(this.form)
				if(success===true){
					uni.showToast({
						title: '添加成功',
						icon: success,
